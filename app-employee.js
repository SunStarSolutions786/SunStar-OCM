...
function renderEmpOrderBody(){
  ...
  const items = (APP.items||[]).filter(i=>i.brand===empBrandFilter);
  const listEl = $('#empItemList');
  if(items.length===0){
    listEl.innerHTML = `<div class="empty-state"><div class="es-icon">📦</div><h4>No items in this brand</h4></div>`;
  } else {
    listEl.innerHTML='';
    items.forEach(it=>{
...
function renderEmpMyOrdersTab(){
  ...
  DB.collection('companies').doc(APP.companyId).collection('orders')
    .where('salesmanName','==',APP.salesman)
    .orderBy('createdAt','desc').limit(100).get().then(snap=>{
      let orders=[];
        ...
        <div class="list-card ${o.status}">
          <div class="lc-top">
            <div>
              ...
              <div class="lc-meta">${fmtDateDisplay(o.date)} · ${escapeHtml(o.brand)} · ${o.items.length} item(s)</div>
            </div>
...
function loadEmpCollection(){
  ...
  DB.collection('companies').doc(APP.companyId).collection('outstanding').doc(empCollDate).get().then(doc=>{
    ...
    const entries = Object.entries(data.outlets||{}).filter(([id,o])=> o.salesmanName===APP.salesman);
    if(entries.length===0){
    ...
    listEl.innerHTML = entries.map(([id,o])=>`
      <div class="list-card">
        <div class="lc-top">
          <div>
            <div class="lc-title">${escapeHtml(o.outletName)}</div>
            <div class="lc-meta">Outstanding: ${fmtINR(o.os)} &nbsp;•&nbsp; Received: ${fmtINR(o.received||0)}</div>
          </div>
        </div>
